...
resource "aws_db_instance" "appdb1" {
  ...
  username                              = var.username
  password                              = random_password.rds_password.result
  vpc_security_group_ids                = [aws_security_group.laalz-secgroup.id, aws_security_group.vpc-secgroup.id, aws_security_group.workspace-secgroup.id]
...
resource "aws_security_group" "workspace-secgroup" {
  name        = "workspace-secgroup"
  description = "RDS Access from workspaces"
  vpc_id      = var.vpc_shared_id
  ...
  tags = {
    Name = "${var.application_name}-${var.environment}-workspace-secgroup"
  }
}
